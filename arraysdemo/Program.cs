...
namespace arraysdemo
{
    ...
    internal class Program
    {
        ...
        static void PrintCustomers(Customer[] custlist)
        {
            Console.WriteLine("\nPrinting the customer array:");
            foreach (Customer cust in custlist)
            {
                if (cust != null)  // Skip null (deleted) customers
                {
        ...
        static void Main(string[] args)
        {
            ...
            for (i = 0; i <= custlist.Length; i++)
            {
                if (custlist[i].customerid==updateid)
                {
                    Console.WriteLine("enter the name to update:");
                    string updatedname1 = Console.ReadLine();
                    custlist[i].customername = updatedname1;
                    updated = true;
                    break;
                }
            }
            ...
            for(i=0;i<=custlist.Length;i++)
            {
                if (custlist[i].customerid==delId)
                {
                    custlist[i] = null; // removing the row
                    deleted=true;
                    break;
            ...
            if (deleted)
            {
                Console.WriteLine("the id as been deleted");
            }
            else
            {
